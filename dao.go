...
type Preempt interface {
	...
	// Preempt 调度失败会进行重试
	Preempt(ctx context.Context, strategy RetryStrategy) (JobSwap, error)
	// SetNextExecTime 设置下次执行时间
	//SetNextExecTime(ctx context.Context, nextExecTime time.Time) error
	//// Release 释放Job
	//Release(job Jobs) error
}
...
// Migrate 迁移Job数据表，需要手动执行
func (j *JobPreempt) Migrate() error {
	return j.db.Migrator().AutoMigrate(&Jobs{})
}

func (j *JobPreempt) TryPreempt(ctx context.Context) (JobSwap, error) {
	var jobs Jobs
	// 状态位执行中且执行时间小于等于当前时间的
	err := j.db.WithContext(ctx).Model(&Jobs{}).
		Where("status = ? AND next_exec_time <= ?",
			_const.SchedulerStatusWaiting, time.Now().Unix()).
		First(&jobs).Error
	if err != nil {
		return JobSwap{}, err
	}

	res := j.db.WithContext(ctx).Model(&Jobs{}).
		Where("id = ? AND epoch = ?", jobs.ID, jobs.Epoch).
...
func (j *JobPreempt) Preempt(ctx context.Context,
	strategy RetryStrategy) (JobSwap, error) {
	...
	for {
		...
		err := j.db.WithContext(lctx).Model(&Jobs{}).
			Where("status = ? AND next_exec_time <= ?",
		...
		lctx, cancel = context.WithTimeout(ctx, time.Second)
		res := j.db.WithContext(lctx).Model(&Jobs{}).
			Where("id = ? AND epoch = ?", jobs.ID, jobs.Epoch).
			Updates(map[string]interface{}{
				"status":     _const.SchedulerStatusRunning,
				"epoch":      jobs.Epoch + 1,
...
// SetNextExecTime 重置下次调度执行的时间
func (j *JobSwap) SetNextExecTime(ctx context.Context, nextExecTime time.Time) error {
	return j.db.WithContext(ctx).
		Model(&Jobs{}).
		Where("id = ? AND epoch = ?", j.JobID, j.Epoch).
		Updates(map[string]interface{}{
